...
public class CoralManipulatorPivot extends SubsystemBase {
    ...
    public void periodic() {
        ...
        if (DriverStation.isEnabled()) {
            ...
            if (hasCoralSupplier.getAsBoolean()) {
                ...
                ff = Constants.CoralManipulator.angleFFCoral.calculate(getAngle(),
                        Constants.CoralManipulator.anglePID.getSetpoint().velocity);
            } else {
                // otherwise it will use the FF for when the manipulator is empty
                ff = Constants.CoralManipulator.angleFFEmpty.calculate(getAngle(),
                        Constants.CoralManipulator.anglePID.getSetpoint().velocity);
            }
    ...
    public double getAngle() {
        return Units.rotationsToRadians(angleMotor.getAbsoluteEncoder().getPosition())
                + Constants.CoralManipulator.FFOffset;
    }

    // public void simulationPeriodic(){
    // armSim.setInput(motorSim.getSetpoint() *
    // RobotController.getBatteryVoltage());
    // //System.out.println(motorSim.getSetpoint());
    // //System.out.println(elevatorSim.getPositionMeters());
    // // In this method, we update our simulation of what our elevator is doing
    // // First, we set our "inputs" (voltages)
    // motorSim.iterate(Units.radiansPerSecondToRotationsPerMinute( // motor
    // velocity, in RPM
    ...
    public class ChangeState extends Command {
        ...
        public boolean isFinished() {
            // System.out.println("coralmanip: " + MathUtil.isNear(getAngle(),
            // targetPosition, Constants.CoralManipulator.angleTolerance));
            return !continuous
                    && MathUtil.isNear(getAngle(), targetPosition, Constants.CoralManipulator.angleTolerance);
        }
    }
    ...
    private class LQR {
        ...
        private final LinearSystem<N2, N1, N2> m_armPlant = LinearSystemId
                .createSingleJointedArmSystem(DCMotor.getNeoVortex(1), kArmMOI, kArmGearing);
        ...
        public void teleopInit() {
            ...
            m_lastProfiledReference = new TrapezoidProfile.State(getAngle(), getRate());
        }
