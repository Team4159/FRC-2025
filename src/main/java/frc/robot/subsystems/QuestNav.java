...
public class QuestNav {
  ...
  public void resetPosition(Pose2d newPose) {
    resetPosition = new Pose2d(getPose().minus(newPose).getTranslation(), getPose().getRotation());
    if (questMiso.get() != 99) {
      questMosi.set(1);
    }
  }
  ...
  private Pose2d getQuestNavPose() {
    var oculousPositionCompensated = getQuestNavTranslation().minus(new Translation2d(0, Units.inchesToMeters(7.5))); // 7.5
